...
## Uses Monte Carlo to find the average uncertainty. 
	...
	for i in range(num_of_scans):
		...
		## Get avg parameters
		dav = np.mean(par[2*i,:])
		aav = np.mean(par[2*i+1,:])	
		## Plot to verify accuracy
		c = next(color)
		plt.figure()
		ax = plt.gca()
		plt.scatter(data[:,2*i],data[:,2*i+1],s=0.5,label = header[0,2*i],color=c)	
		plt.plot(data[:,2*i],func((data[:,2*i],n(polymer,temp[:,2*i]),0),dav,aav),c=c)
		plt.text(0.5,0.9-i*0.05, "d = {0} nm, a = {1}".format(np.around(dav,2),np.around(aav,2)),transform=ax.transAxes)
