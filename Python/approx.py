...
## to then adjust to match model better
	...
	for j in range(num_of_scans):
		...
		extwav = np.concatenate((minwav,maxwav))
		extabs = np.concatenate((minabs,maxabs))
			
		## Plot raw and smoothed data, plus extrema
		plt.figure()
		plt.scatter(temp1[:,2*j],temp1[:,2*j+1],label="Data",s=0.5)
		plt.plot(temp1[:,2*j],temp2,label="Smoothed",c="tab:orange")
		plt.scatter(extwav,extabs,c="b",label="Extrema")
		plt.legend()
		plt.show()
		...
		## And to print out the thickness
...
def main():
	...
	for i in range(num_of_scans):
		print("{0} thickness = ".format(header[0,2*i]), np.around(t[i],2)," nm")
